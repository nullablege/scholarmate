...
if(mysqli_num_rows($result) == 1){
    ...
    $query = "select * from makale_paylasim where paylasilan_id='$id';";
    $result = mysqli_query($conn,$query);
    if(!mysqli_num_rows($result) == 1){
        //header("location:index.php");
    }
            ...
            <div class="article-detail-card animate-fade-in-down">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div class="d-flex gap-2">
                        <a href="/includes/disaAktar.php?id=<?php echo $duzenlenecek_id ?>" target="_blank"><button class="btn btn-sm btn-outline-custom" title="PDF Olarak Dışa Aktar"><i class="bi bi-file-earmark-pdf-fill me-1"></i> Dışa Aktar</button></a>
                        <a href="MakaleDuzenle.php?id=<?php echo $makale_detay['id']; ?>" class="btn btn-sm btn-action-filled" title="Makaleyi Düzenle"><i class="bi bi-pencil-square me-1"></i> Düzenle</a>
                        
                        <button class="btn btn-sm btn-danger" title="Makaleyi Sil" onclick="makaleyiSilAjax(<?php echo $duzenlenecek_id; ?>, '<?php echo $duzenlenecek_id; ?>')">
                ...
                <div class="tab-content" id="articleTabContent">
                    ...
                    <div class="tab-pane fade" id="summary-tab-pane" role="tabpanel" aria-labelledby="summary-tab" tabindex="0">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <div>
                                <button class="btn btn-sm btn-action-filled" type="button" id="eskiOzetOto" title="Özeti Manuel Düzenle"><i class="bi bi-pencil-fill"></i> Eski Özete Dön</button>
                                <form style="display: inline;" action="includes/yeniOzet.php" method="POST">
                                <button class="btn btn-sm btn-outline-custom me-2" type="submit" title="Otomatik Özeti Yeniden Oluştur"><i class="bi bi-arrow-repeat"></i> Yeniden Oluştur</button>
                                <input type="hidden" name="makaleId2" value="<?php echo $duzenlenecek_id;?>">   
                                </form>
                                <form style="display: inline;" action="includes/ozetDuzenle.php" method="POST">
                                <button class="btn btn-sm btn-action-filled" type="submit" title="Özeti Manuel Düzenle"><i class="bi bi-pencil-fill"></i> Kaydet</button>
                                
                            </div>
                        </div>
                        <div class="section-content p-3 rounded" style="background-color: var(--input-bg); border: 1px solid var(--input-border);">
                            <textarea  name="manuelOzet" id="yeniOzet" rows="6" class="form-control" style="resize: vertical;"><?php echo htmlspecialchars($makale_detay['otomatik_ozet']); ?></textarea>
                            <input type="hidden" name="makaleId" value="<?php echo $duzenlenecek_id;?>">
                            <input type="hidden" name="eskiOzet" value="<?php echo $makale_detay['eski_ozet']; ?>">
                        </div>
                    </div>
                    </form>
                    ...
                    <div class="tab-pane fade" id="notes-tab-pane" role="tabpanel" aria-labelledby="notes-tab" tabindex="0">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <button class="btn btn-sm btn-action-filled" data-bs-toggle="modal" data-bs-target="#addNoteModal"><i class="bi bi-plus-lg"></i> Yeni Not Ekle</button>
                        </div>
                        <div class="section-content">
                                ...
                                <div class="section-content">
                                    ...
                                    <div class="search-container mb-3">
                                        <div class="input-group">
                                            <input type="text" name="eklenecekMail" class="form-control" id="userSearchInput" placeholder="E-posta veya kullanıcı adı ile ara..." autocomplete="off" style="background-color: var(--input-bg); color:var(--text-color); border-color:var(--input-border);">
                                            <input type="hidden" name="makaleId" value="<?php echo $duzenlenecek_id; ?>">
                                            ...
                                            <button class="btn btn-action-filled" type="submit" onclick="addUserToShare()"><i class="bi bi-person-plus-fill me-1"></i>Kullanıcı Ekle</button>
                                        </div>
                                ...
                                <div id="sharedUserList">
                                    ...
                                    <?php else: ?>
                                        ...
                                        <div class="user-pill">
                                            ...
                                            $pid = $paylasilan['id'];
                                            ?>
                                            <a href="<?php echo "includes/paylasimSil.php?makale=$duzenlenecek_id&id=$pid"?>"><button class="remove-user-btn" type="button" title="Paylaşımı Kaldır"><i class="bi bi-x-lg"></i></button></a>
                                            </div>
                                        <?php endwhile; ?>
                                    <?php endif; ?>
                                </div>
                ...
                <div class="modal-body">
                    ...
                    <form action="includes/notEkle.php"id="addNoteForm" method="POST">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="icerik" id="noteContent" rows="5" required style="background-color:var(--input-bg); color:var(--text-color); border-color:var(--input-border);"></textarea>
                                        <input type="hidden" name="makaleid" value="<?php echo $duzenlenecek_id; ?>">
                        </div>
                        <div class="mb-3">
                            <label for="noteReference" class="form-label">Referans (Sayfa/Paragraf - Opsiyonel):</label>
                            <input type="text"  name="referans "class="form-control" id="noteReference" placeholder="Örn: s.12, p.3" style="background-color:var(--input-bg); color:var(--text-color); border-color:var(--input-border);">
                        </div>
...
function makaleyiSilAjax(makaleId, satirId) {
    if (confirm('Bu makaleyi silmek istediğinizden emin misiniz?')) {
        fetch('includes/makale_sil_ajax.php', { 
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded', 
            },
            body: 'makale_id=' + makaleId + '&sil_onay=1' 
        })
        .then(response => {
            if (!response.ok) {
                return response.text().then(text => { throw new Error('Sunucu hatası: ' + response.status + " " + text) });
            }
            return response.json(); 
        })
        .then(data => {
            if (data.success) {
                alert(data.message || 'Makale başarıyla silindi.');
                ...
                alert('Hata: ' + (data.message || 'Makale silinirken bir sorun oluştu.'));
            }
        })
        .catch(error => {
            console.error('Silme işlemi sırasında AJAX hatası:', error);
